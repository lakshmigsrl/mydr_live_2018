<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Homepage'), ['action' => 'edit', $homepage->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Homepage'), ['action' => 'delete', $homepage->id], ['confirm' => __('Are you sure you want to delete # {0}?', $homepage->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Homepages'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Homepage'), ['action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Articles'), ['controller' => 'Articles', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Article'), ['controller' => 'Articles', 'action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="homepages view large-9 medium-8 columns content">
    <h3><?= h($homepage->title) ?></h3>
    <table class="vertical-table">
        <tr>
            <th><?= __('Article') ?></th>
            <td><?= $homepage->has('article') ? $this->Html->link($homepage->article->title, ['controller' => 'Articles', 'action' => 'view', $homepage->article->id]) : '' ?></td>
        </tr>
        <tr>
            <th><?= __('Title') ?></th>
            <td><?= h($homepage->title) ?></td>
        </tr>
        <tr>
            <th><?= __('Id') ?></th>
            <td><?= $this->Number->format($homepage->id) ?></td>
        </tr>
        <tr>
            <th><?= __('Start') ?></th>
            <td><?= h($homepage->start) ?></td>
        </tr>
        <tr>
            <th><?= __('End') ?></th>
            <td><?= h($homepage->end) ?></td>
        </tr>
        <tr>
            <th><?= __('Created') ?></th>
            <td><?= h($homepage->created) ?></td>
        </tr>
        <tr>
            <th><?= __('Modified') ?></th>
            <td><?= h($homepage->modified) ?></td>
        </tr>
    </table>
    <div class="row">
        <h4><?= __('Body') ?></h4>
        <?= $this->Text->autoParagraph(h($homepage->body)); ?>
    </div>
</div>
