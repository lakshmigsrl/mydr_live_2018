              ...
              if (current.name == $inputAj.val()) {
                  ...
                  $("#relatedArticlesUl")
                      .append("<li id='li2_relArt_"+current.id+"'>"
                                +"<input type='hidden' value='"+current.name+"' name='dummy_related_articles["+current.id+"]' />"
                                    +"<a target='blank' href='/"+current.section_url+"/"+current.url+"'>"+current.name+"</a>"
                                    +"<a href='#relatedArticlesUl' onclick=\"$('#li2_relArt_"+current.id+"').remove();\" style='color: #990000;'><i> [delete] </i></a>"
                              +"</li>");
                  $inputAj.val('');
              } else {
