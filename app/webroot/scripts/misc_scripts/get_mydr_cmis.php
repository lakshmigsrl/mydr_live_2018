...
require_once '/var/www/prod/cakemydr/mydr.com.au/config/database.php';
   ...
   $dbPassword = $dbVars['default']['password'];
	...
	if($op=="none"){
		...
		echo "<h1>myDr CMIs list</h1>";
		echo "<form action='get_mydr_cmis.php' method='post'>";
		...
		while($currRow = $result->fetch_array(MYSQLI_ASSOC)){
			$sqlUrl = "SELECT lpd.DocumentName as cmi_code, lpd.Description as iDesc ".
			...
			while($dRow = $resUrl->fetch_array(MYSQLI_ASSOC)){
				$toPush = $currRow;
				if($resUrl_numRows>1){
					$multicmiUrl = str_replace($omitChars, '', trim($dRow['iDesc']));
				...
				$newRow = array_merge($toPush, array('URL'=>$toPush['URL'], 'cmi_code'=>$dRow['cmi_code']));
