...
require_once '/var/www/prod/cakemydr/mydr.com.au/config/database.php';
   ...
   $dbPassword = $dbVars['default']['password'];
	...
	if($op=="none"){
		...
		$sqlStr = "SELECT COUNT(*) AS Total FROM `users` WHERE  status=1 and newsletters=1 and thirdpartyoffers=1";
		$result = mysql_query($sqlStr);
		while($row = mysql_fetch_array($result, MYSQL_ASSOC)){
			$total = $row['Total'];
		}
		echo "<h1>myDr's mailing list with thirdparty set List</h1>";
		echo "<form action='get_mydr_thirdparty.php' method='post'>";
		echo "<input type='hidden' name='op' value='yes' />";
		echo "Total subscribers: ".$total.".  <input type='button' value='Get CSV file...' onclick='submit();'> ";
		echo "</form>";
	}else{

		$sqlStr = "SELECT fname, lname, email, email_status, thirdpartyoffers, state, country, gender, type, status, "
			 ."MD5(CONCAT(login, '-', email)) AS  confirm_hash "
			 ." FROM `users` "
			 ." WHERE status=1 and newsletters=1 and thirdpartyoffers=1 ";
		$datedfname = "myDrMailinglist_thirdparty_".date("dMY").".csv";
